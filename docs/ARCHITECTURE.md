...
> 這份文件是 [README](../README.md) 的深度版：完整的重構論述、架構總覽、新增功能與已知限制。
> README 是作品集門面，只放精選重點與指路；細節都在這裡。

## 目錄

- [重構亮點（完整版）](#refactor-highlights)
- [架構總覽](#architecture-overview)
  - [Models](#models)
  - [文字版 ERD](#erd)
  - [刻意保留的非正規設計](#non-normalized-design)
  - [路由設計](#routing)
  - [前端：全 Hotwire／Tailwind，零 jQuery、零 CSS 框架 CDN](#frontend)
  - [CSS 編譯器](#css-compiler)
- [相較原作的新增功能](#new-features)
- [已知限制（完整版）](#known-limitations)
- [部署與維運細節](#deployment-details)

<a id="refactor-highlights"></a>
## 重構亮點（完整版）
...
`ScoreEntry`。計分邏輯有黃金值測試護欄：`test/integration/game/scoring_golden_test.rb`
把整條鏈（timer → 提示×N → answer → ready → attack 擊敗 → score）跑在凍結時間上，
6 個分項逐一寫死字面量，schema 正規化前後斷言值逐位元相同；完整的正規化決策表見
`docs/SCHEMA_REDESIGN.md`。

<a id="non-normalized-design"></a>
### 刻意保留的非正規設計
...
- **Boss 戰遊戲化**：點怪物本體攻擊、受擊震動與傷害數字、連擊計數、HP 條
  ...
  見[重構亮點](#refactor-highlights)）。第 10/11 關依原始劇情考證還原為「摩天輪魔王」雙型態連戰。
- **四職業主動技**（每場王戰每人一次，`BossSkillUse` unique index 保證；
  ...
  設計討論全文見 `docs/JOB_SKILLS_DESIGN.md`。
- **營運後台**（`/admin`，全區伺服器端驗證）：營運總覽 Dashboard（進度
  ...
  **答案重設**——輸入明文伺服器轉 digest、永不回顯）、兌獎序號池管理與
  批次產生、隊伍序號產生器（rqrcode QR）。功能清單與實作順序見
  `docs/ADMIN_CONSOLE_PLAN.md`（A1-A5）。
...
[`docs/DEPLOYMENT.md`](DEPLOYMENT.md)；定價與方案選擇的查證依據見
...
`db/seeds.rb` 保留的固定序號 `Team::DEMO_SERIAL_NO`（見 `app/models/team.rb`）
只是展示資料本身、**刻意不預塞任何玩家**，跟首頁「▶ 直接體驗（Demo）」按鈕
（`POST /game/session` 帶 `demo: "1"`，見 `Game::SessionsController#create_demo!`）
每次點擊當場建立的全新單人示範隊伍是兩回事——後者才是訪客實際登入體驗的隊伍。
已登入 demo 隊伍的頁面（`/game`）會在主選單標題旁顯示一個「Demo」標籤；示範
資料把所有題目的王戰 HP 設定成 10、基準時限 60 秒（隊上有「阿北」職業時
+10 秒），方便單人在合理時間內打完全部 Boss。
